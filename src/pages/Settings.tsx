      ...
      <Tabs defaultValue="profile">
        <TabsList className="mb-4 flex-wrap">
          <TabsTrigger value="profile">
            <User className="mr-1 inline h-3 w-3" /> Profile
          </TabsTrigger>
          <TabsTrigger value="simulation">
            <FlaskConical className="mr-1 inline h-3 w-3" /> Simulation
          </TabsTrigger>
          ...
          <TabsTrigger value="appearance">
            <Palette className="mr-1 inline h-3 w-3" /> Appearance
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="profile">
          <Card className="max-w-xl">
            ...
            <CardContent className="space-y-3">
              <div>
                <label className="mb-1 block text-[11px] text-ink-muted">Full Name</label>
                <Input defaultValue="Jordan Reyes" />
              </div>
              <div>
                <label className="mb-1 block text-[11px] text-ink-muted">Role</label>
                <Input defaultValue="Medical Device Systems Engineer" />
              </div>
              <div>
                <label className="mb-1 block text-[11px] text-ink-muted">Organization</label>
                <Input defaultValue="MedTwin Labs" />
              </div>
              <div>
                <label className="mb-1 block text-[11px] text-ink-muted">Email</label>
                <Input defaultValue="jordan.reyes@medtwin.dev" />
              </div>
              <Button
                variant="primary"
                onClick={() => pushToast({ title: "Profile saved", tone: "safe" })}
              >
          ...
          <Card className="max-w-xl">
            ...
            <CardContent className="divide-y divide-hairline">
              <Row label="Tick Rate" description="Simulation clock resolution">
                <Select
                  value={simSettings.tickRate}
                  onChange={(v) => setSimSettings((s) => ({ ...s, tickRate: v }))}
                  className="w-32"
                  options={[
              ...
              <Row label="Default Fidelity" description="Digital twin model resolution">
                <Select
                  value={simSettings.fidelity}
                  onChange={(v) => setSimSettings((s) => ({ ...s, fidelity: v }))}
                  className="w-32"
                  options={[
                    { value: "low", label: "Low" },
              ...
              <Row
                ...
                description="Halt simulation automatically if safety score drops below 20"
              >
                <Toggle
                  checked={simSettings.autoStop}
                  onChange={() => setSimSettings((s) => ({ ...s, autoStop: !s.autoStop }))}
                />
              </Row>
            </CardContent>
          </Card>
            ...
            <CardContent className="divide-y divide-hairline">
              <Row label="Analysis Model" description="Model used for safety scoring and findings">
                <Select
                  ...
                  onChange={(v) => setAiSettings((s) => ({ ...s, model: v }))}
                  className="w-48"
                  options={[
                    { value: "medtwin-safety-v2", label: "MedTwin Safety v2" },
                    { value: "medtwin-safety-v1", label: "MedTwin Safety v1" },
                  ]}
                />
              </Row>
              <Row
                label="Confidence Threshold"
                description="Minimum confidence to surface a finding"
              >
                <Input
                  value={aiSettings.confidenceThreshold}
                  onChange={(e) =>
                    setAiSettings((s) => ({ ...s, confidenceThreshold: e.target.value }))
                  }
                  className="w-20"
                />
              </Row>
            ...
            <CardContent className="divide-y divide-hairline">
              ...
              <Row label="Warnings" description="Notify on warning-level anomalies">
                <Toggle
                  ...
                  onChange={() => setNotifications((n) => ({ ...n, warnings: !n.warnings }))}
                />
              </Row>
              ...
              <Row label="Weekly digest" description="Summary of fleet-wide safety trends">
                <Toggle
                  ...
                  onChange={() => setNotifications((n) => ({ ...n, digest: !n.digest }))}
                />
              </Row>
            </CardContent>
          </Card>
        ...
        <TabsContent value="appearance">
          <Card className="max-w-xl">
            ...
            <CardContent className="space-y-3">
              <Row
                label="Theme"
                description="MedTwin currently ships a single dark laboratory theme"
              >
                <Select
                  value={theme}
                  onChange={setTheme}
                  className="w-40"
                  options={[{ value: "dark-lab", label: "Dark Lab" }]}
                />
              </Row>
              <div className="rounded-md border border-hairline bg-panel-raised p-3">
                ...
                <p className="mt-1 text-[11px] text-ink-muted">
                  MedTwin currently runs on local mock data. Connect a FastAPI backend at this
                  address to enable live device telemetry.
                </p>
              </div>
            </CardContent>
          </Card>
