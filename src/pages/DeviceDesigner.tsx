...
import { CircuitBoard, Radio, Cpu, Zap, Battery, ShieldCheck, Save, CheckCircle2, AlertTriangle } from "lucide-react";
...
export default function DeviceDesigner() {
  ...
  const device = useMemo(() => devices.find((d) => d.id === selectedDeviceId) ?? devices[0], [devices, selectedDeviceId]);
  const [localDevice, setLocalDevice] = useState<Device>(device);
  const [selectedComponentId, setSelectedComponentId] = useState<string | null>(device.components[0]?.id ?? null);
  ...
  const selectedComponent = localDevice.components.find((c) => c.id === selectedComponentId) ?? null;
  ...
  function onPointerDown(e: React.PointerEvent, comp: DeviceComponent) {
    ...
    dragState.current = { id: comp.id, offsetX: e.clientX - rect.left - comp.x, offsetY: e.clientY - rect.top - comp.y };
  ...
  function onPointerMove(e: React.PointerEvent) {
    ...
    const x = Math.max(0, Math.min(rect.width - 140, e.clientX - rect.left - dragState.current.offsetX));
    const y = Math.max(0, Math.min(rect.height - 70, e.clientY - rect.top - dragState.current.offsetY));
  ...
  return (
    <AppLayout title="Device Designer" subtitle="Compose the block-level architecture of the device under development">
      <SectionHeader
      ...
      {validation && (
        <div className={`mb-4 flex items-start gap-2.5 rounded-lg border p-3 text-sm ${validation.valid ? "border-safe/30 bg-safe/5 text-safe" : "border-amber/30 bg-amber/5 text-amber"}`}>
          {validation.valid ? <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0" /> : <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />}
          <div>
            <p className="font-medium">{validation.valid ? "Device passed validation" : "Validation found issues"}</p>
            {!validation.valid && (
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1fr_320px]">
        <Card className="overflow-hidden">
          <CardHeader>
            ...
            <span className="flex items-center gap-1 text-[11px] text-ink-faint"><CircuitBoard className="h-3.5 w-3.5" /> drag blocks to reposition</span>
          </CardHeader>
          <div
            ref={canvasRef}
          ...
          >
            <svg className="pointer-events-none absolute inset-0 h-full w-full">
              {localDevice.connections.map((conn) => {
                ...
                const x1 = from.x + 70, y1 = from.y + 35, x2 = to.x + 70, y2 = to.y + 35;
                const midX = (x1 + x2) / 2;
                return (
                  <path
                    key={conn.id}
            ...
            {localDevice.components.map((comp) => {
              ...
              return (
                <div
                  ...
                  style={{ left: comp.x, top: comp.y, borderColor: active ? meta.color : undefined }}
                ...
                >
                  <div className="flex items-center gap-1.5">
                    <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded" style={{ backgroundColor: `${meta.color}1A` }}>
                      <Icon className="h-3.5 w-3.5" style={{ color: meta.color }} />
                    </div>
                    <span className="truncate text-[11px] font-medium text-ink">{comp.label}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-[9px] uppercase tracking-wide text-ink-faint">{comp.kind}</span>
                    <Badge tone={statusTone(comp.status)} className="px-1.5 py-0 text-[9px]">{comp.status}</Badge>
                  </div>
                </div>
              );
            })}
            ...
            {selectedComponent ? (
              <>
                <div>
                  ...
                  <Input value={selectedComponent.label} onChange={(e) => updateComponent(selectedComponent.id, { label: e.target.value })} />
                </div>
                <div>
                  <label className="mb-1 block text-[11px] font-medium text-ink-muted">Status</label>
                  <Select
                    value={selectedComponent.status}
                    onChange={(v) => updateComponent(selectedComponent.id, { status: v as DeviceComponent["status"] })}
                    options={[
                      { value: "ok", label: "OK" },
                ...
                <div className="border-t border-hairline pt-3">
                  <p className="mb-2 text-[11px] font-medium uppercase tracking-wide text-ink-faint">Properties</p>
                  <div className="space-y-2">
                    {Object.entries(selectedComponent.properties).map(([key, value]) => (
                      <div key={key}>
                        <label className="mb-1 block text-[11px] capitalize text-ink-muted">{key}</label>
                        <Input value={String(value)} onChange={(e) => updateProperty(key, e.target.value)} />
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <p className="text-xs text-ink-muted">Select a component on the canvas to edit its properties.</p>
            )}

            <div className="border-t border-hairline pt-3">
              <p className="mb-2 text-[11px] font-medium uppercase tracking-wide text-ink-faint">Component Palette</p>
              <div className="grid grid-cols-3 gap-2">
                {(Object.keys(KIND_META) as ComponentKind[]).map((kind) => {
                  const meta = KIND_META[kind];
                  const Icon = meta.icon;
                  return (
                    <div key={kind} className="flex flex-col items-center gap-1 rounded-md border border-hairline bg-panel-raised py-2">
