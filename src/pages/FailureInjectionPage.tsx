    ...
    <AppLayout>
      ...
      <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <Card className="p-4">
          ...
          <div className="mt-2">
            <Badge tone={statusTone(riskLevel)} className="text-xs">
              {riskLevel}
            </Badge>
          </div>
        </Card>
          ...
          <Card key={f.id} className={`p-4 transition-colors ${f.active ? "border-amber/40" : ""}`}>
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-start gap-2.5">
                <div
                  className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-md border ${f.active ? "border-amber/40 bg-amber/10" : "border-hairline bg-panel-raised"}`}
                >
            ...
            <div className="mt-3">
              ...
              <input
                ...
                style={{
                  background: `linear-gradient(to right, #F5A623 ${f.intensity}%, #212B37 ${f.intensity}%)`,
                }}
              />
            </div>
          </Card>
      ...
      {activeCount > 2 && (
        <div className="mt-4 flex items-start gap-2.5 rounded-lg border border-critical/30 bg-critical/5 p-3 text-sm text-critical">
          ...
          <p>
            Multiple concurrent failures active — this compound scenario significantly increases
            cumulative risk. Review AI Safety Analysis for details.
          </p>
        </div>
      )}
    </AppLayout>
