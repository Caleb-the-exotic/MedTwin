...
export default function DatasetExplorer() {
  ...
  const labels = useMemo(
    () => ["all", ...Array.from(new Set(initialDataset.map((d) => d.label)))],
    [],
  );

  const filtered = initialDataset.filter((d) => {
    const matchesQuery =
      d.id.toLowerCase().includes(query.toLowerCase()) || d.label.includes(query.toLowerCase());
  ...
  const avgSpo2 =
    Math.round((initialDataset.reduce((a, d) => a + d.spo2, 0) / initialDataset.length) * 10) / 10;
    ...
    <AppLayout>
      <div className="mb-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <StatCard
          icon={Database}
          label="Total Records"
          value={initialDataset.length}
          tone="signal"
        />
        <StatCard
          icon={AlertTriangle}
          label="Anomalies Labeled"
          value={anomalyCount}
          tone="amber"
        />
        <StatCard
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <Select
              ...
              className="w-40"
              options={labels.map((l) => ({ value: l, label: l === "all" ? "All Labels" : l }))}
            />
          </div>
        }
      />
          ...
          <TBody>
            {filtered.slice(0, 30).map((d) => (
              <TR key={d.id}>
                ...
                <TD>
                  <Badge tone={d.anomaly ? "amber" : "safe"}>{d.label}</Badge>
                </TD>
              </TR>
            ))}
          </TBody>
